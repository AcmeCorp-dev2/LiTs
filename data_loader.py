...
class LITS(torch.utils.data.Dataset):
    ...
    def __init__(self, root, size=(512, 512), transform=None, train=True):
        ...
        if (transform==None):
            self.transform = Compose([Resize(self.size),ToTensor()])
        else:
...
def loader(dataset, batch_size, num_workers=8, shuffle=True):
    ...
    input_loader = DataLoader(dataset=input_images,batch_size=batch_size,shuffle=shuffle, num_workers=num_workers)
